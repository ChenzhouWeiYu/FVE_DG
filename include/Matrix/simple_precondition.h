...
////////下面是加循环后编译可以通过，但是爆内存的代码，增加 LongVector<BlockDim> Urme_tmp = rhs; 不爆内存了
 ...
 Vec apply(const Vec& rhs, uInt max_sweeps = 100, Scalar epsilon = 1e-6) {   
    ...
    std::cout << "[没有进入sweep循环时整体系统残差] || A * U - b || = " << res_all0.norm()/rhs.norm() << std::endl;
    // return Urme_tmp;
    // 可调迭代次数
    // constexpr int max_sweeps = 100;
    constexpr Scalar sor_weight_r = 0.125;
    ...
    for (uInt sweep = 0; sweep < max_sweeps; ++sweep) {
        ...
        Arr_solver.set_rhs(rhs_rho);
        Ur_tmp = (1-sor_weight_r) * Ur_tmp + sor_weight_r * Arr_solver.SparseLU(Ur_tmp);
        set_sub_vector<NumBasis,1,0>(Ur_tmp, Urme_tmp);

        // ------- 整体系统残差 -------
        // Vec AU = A.multiply(Urme_tmp);
        Scalar res_r = (A.multiply(Urme_tmp) - rhs).norm()/rhs.norm();
        std::cout << "[更新密度后计算整体系统残差] || A * U - b || = " << res_r << std::endl;  //暂时注释掉
        if(res_r < epsilon) break;
        ...
        LongVector<3*NumBasis> rhs_m = bm
            ...
            - Ame.multiply(Ue_tmp);
        Amm_solver.set_rhs(rhs_m);
        Um_tmp = (1-sor_weight_m) * Um_tmp + sor_weight_m * Amm_solver.SparseLU(Um_tmp);
        set_sub_vector<NumBasis,3,1>(Um_tmp, Urme_tmp);
        
        // ------- 整体系统残差 -------
        Scalar res_m = (A.multiply(Urme_tmp) - rhs).norm()/rhs.norm();
        std::cout << "[更新动量后计算整体系统残差] || A * U - b || = " << res_m << std::endl;  //暂时注释掉
        if(res_m < epsilon) break;
        ...
        LongVector<1*NumBasis> rhs_e = be
            ...
            - Aem.multiply(Um_tmp);
        Aee_solver.set_rhs(rhs_e);
        Ue_tmp = (1-sor_weight_e) * Ue_tmp + sor_weight_e * Aee_solver.SparseLU(Ue_tmp);
        set_sub_vector<NumBasis,1,4>(Ue_tmp, Urme_tmp);

        // ------- 整体系统残差 -------
        Scalar res_e = (A.multiply(Urme_tmp) - rhs).norm()/rhs.norm();
        std::cout << "[更新能量后计算整体系统残差] || A * U - b || = " << res_e << std::endl;  //暂时注释掉
        if(res_e < epsilon) break;
    }
...
//     Vec AU = A.multiply(Urme_tmp);
//     Vec res_all = AU - rhs;
//     std::cout << "[更新密度后计算整体系统残差] || A * U - b || = " << res_all.norm()/rhs.norm() << std::endl;
...
//     std::cout << "[更新动量后计算整体系统残差] || A * U - b || = " << res_all2.norm()/rhs.norm() << std::endl;
...
//     std::cout << "[更新能量后计算整体系统残差] || A * U - b || = " << res_all3.norm()/rhs.norm() << std::endl;
// }
       // ------- 整体系统残差 -------
    Vec AU = A.multiply(Urme_tmp);
    Vec res_all = AU - rhs;
