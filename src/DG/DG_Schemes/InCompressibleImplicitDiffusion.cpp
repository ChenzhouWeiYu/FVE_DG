#include "DG/DG_Schemes/InCompressibleImplicitDiffusion.h"

#define Explicit_For_Flux(Order) \
template class InCompressibleImplicitDiffusion<Order+1,Order,AirFluxD>;\
template class InCompressibleImplicitDiffusion<Order+1,Order,MonatomicFluxD>;\
template class InCompressibleImplicitDiffusion<Order+2,Order+1,AirFluxD>;\
template class InCompressibleImplicitDiffusion<Order+2,Order+1,MonatomicFluxD>;\
template class InCompressibleImplicitDiffusion<Order+3,Order+2,AirFluxD>;\
template class InCompressibleImplicitDiffusion<Order+3,Order+2,MonatomicFluxD>;
...
void InCompressibleImplicitDiffusion<OrderU, OrderP, Flux, GaussQuadCell, GaussQuadFace>::
    ...
    */
    ...
    for(uInt fid = 0; fid < mesh.m_faces.size(); ++fid) {
        ...
        for(uInt g = 0; g < QuadF::num_points; ++g) {
            ...
            const auto& u_grads = BasisU::grad_all(xi[0], xi[1], xi[2]);
            ...
            case BoundaryType::Dirichlet: {
                ...
                uvw_bc = uvw_Xi(cell, xi, curr_time); 
                p_bc = p_Xi(cell, xi, curr_time);
            ...
            /* Lax-Friedrichs参数：λ = max(|u_in·n|, |u_bc·n|)
               平均状态：u_avg = 0.5(u_in + u_bc)
                        p_avg = 0.5(p_in + p_bc) */
            const Scalar penalty = OrderU*OrderU/(cell.m_h); // 惩罚项
            // 计算所有基函数的物理坐标梯度
            std::array<vector3f, NU> grad_phi;
            for(uInt bid = 0; bid < NU; ++bid) {
                grad_phi[bid] = cell.get_invJacMat().multiply(u_grads[bid]);
            }
            const DenseMatrix<3,1> uvw_avg = 0.5 * (uvw_inner + uvw_bc);
            const Scalar p_avg = 0.5 * (p_inner + p_bc);
            // 动量方程通量项 -------------------------------------------------
            for(uInt j = 0; j < NU; ++j) { // 测试函数ϕ_j
                const auto& grad_phi_j = grad_phi[j]; // ∇ϕ_j
                const auto& SIP_S_kernel = 0.5 * compute_stress_kernel(grad_phi_j, normal);
                const auto& SIP_P_kernel = 0.5 * u_basis[j] * penalty * compute_stress_kernel(normal, normal);
                    
                for(uInt i = 0; i < NU; ++i) { // 试探函数ϕ_i
                    const auto& grad_phi_i = grad_phi[i]; // ∇ϕ_i
                    // SIP 
                    const auto& SIP_kernel = 0.5 * u_basis[j] * compute_stress_kernel(normal, grad_phi_i);
                    assemble_Auu(face_matrix, 
                               (SIP_kernel + u_basis[i] * (SIP_S_kernel - SIP_P_kernel)) * jac_weight,
                               j, i);
                }
                assemble_bu(face_rhs, (SIP_S_kernel - SIP_P_kernel).multiply(uvw_bc) * jac_weight, j);
            }
              ...
              BlockSparseMatrix<DoFs,DoFs>& sparse_mat) 
{
    ...
    for(uInt cid = 0; cid < mesh.m_cells.size(); ++cid) {
        ...
        for(uInt g = 0; g < num_vol_points; ++g) {
            ...
            std::array<vector3f, NU> grad_phi;
            for(uInt bid = 0; bid < NU; ++bid) {
                grad_phi[bid] = invJ.multiply(u_grads[g][bid]); // ∇ϕ_i = J^{-T}∇̂ϕ_j
            }

            // 组装动量方程
            for(uInt j = 0; j < NU; ++j) { // 试探函数ϕ_j
                const auto& grad_phi_j = grad_phi[j]; // ∇ϕ_j = J^{-T}∇̂ϕ_j
                for(uInt i = 0; i < NU; ++i) { // 测试函数ϕ_i
                    const auto& grad_phi_i = grad_phi[i]; // ∇ϕ_i = J^{-T}∇̂ϕ_i
                    // 计算粘性应力核
                    const auto& viscous_kernel = compute_stress_kernel(grad_phi_j, grad_phi_i);
                    assemble_Auu(cell_matrix, 
                                - viscous_kernel * jac_weight,
                               j, i); // 注意i,j顺序
                }
            }
        }
        #pragma omp critical
        sparse_mat.add_block(cid, cid, - cell_matrix); // 离散的是负的扩散项 -\nabla\cdot\tau 
    }
}
                       ...
                       BlockSparseMatrix<DoFs,DoFs>& sparse_mat) 
{
    ...
    for(uInt fid = 0; fid < mesh.m_faces.size(); ++fid) {
        ...
        const auto& cell_L = mesh.m_cells[L];      // 左单元
        const auto& cell_R = mesh.m_cells[R];      // 右单元
        ...
        for(uInt g = 0; g < QuadF::num_points; ++g) {
            ...
            // 转换到单元局部坐标
            const auto& xi_L = transform_to_cell(face, uv, 0);
            const auto& xi_R = transform_to_cell(face, uv, 1);

            // 计算基函数值
            const auto& phi_L = BasisU::eval_all(xi_L[0], xi_L[1], xi_L[2]);
            const auto& phi_grads_L = BasisU::grad_all(xi_L[0], xi_L[1], xi_L[2]);
            const auto& phi_R = BasisU::eval_all(xi_R[0], xi_R[1], xi_R[2]);
            const auto& phi_grads_R = BasisU::grad_all(xi_R[0], xi_R[1], xi_R[2]);
            const auto& psi_L = BasisP::eval_all(xi_L[0], xi_L[1], xi_L[2]);
            const auto& psi_R = BasisP::eval_all(xi_R[0], xi_R[1], xi_R[2]);
            ...
            // 计算所有基函数的物理坐标梯度
            std::array<vector3f, NU> grad_phi_L, grad_phi_R;
            for(uInt bid = 0; bid < NU; ++bid) {
                grad_phi_L[bid] = cell_L.get_invJacMat().multiply(phi_grads_L[bid]); // ∇ϕ_i = J^{-T}∇̂ϕ_i
                grad_phi_R[bid] = cell_R.get_invJacMat().multiply(phi_grads_R[bid]); // ∇ϕ_i = J^{-T}∇̂ϕ_i
            }

            Scalar penalty = OrderU*OrderU/(cell_L.m_h+cell_R.m_h); // 惩罚项
            // 组装通量项
            for(uInt j = 0; j < NU; ++j) { // 测试函数ϕ_j
                const auto& grad_phi_j_L = grad_phi_L[j]; // ∇ϕ_j^L
                const auto& grad_phi_j_R = grad_phi_R[j]; // ∇ϕ_j^R
                for(uInt i = 0; i < NU; ++i) { // 试探函数ϕ_i
                    const auto& grad_phi_i_L = grad_phi_L[i]; // ∇ϕ_i^L
                    const auto& grad_phi_i_R = grad_phi_R[i]; // ∇ϕ_i^R
                    /* 扩散 SIP  一致项 */
                    const auto& SIP_LL_kernel = 0.5 * phi_L[j] * compute_stress_kernel(normal, grad_phi_i_L);
                    const auto& SIP_LR_kernel = 0.5 * phi_L[j] * compute_stress_kernel(normal, grad_phi_i_R);
                    const auto& SIP_RL_kernel = 0.5 * phi_R[j] * compute_stress_kernel(-normal, grad_phi_i_L);
                    const auto& SIP_RR_kernel = 0.5 * phi_R[j] * compute_stress_kernel(-normal, grad_phi_i_R);
                    /* 扩散 SIP  对称项 */
                    const auto& SIP_LL_S_kernel = 0.5 * phi_L[i] * compute_stress_kernel(grad_phi_j_L, normal);
                    const auto& SIP_LR_S_kernel = 0.5 * phi_R[i] * compute_stress_kernel(grad_phi_j_L, -normal);
                    const auto& SIP_RL_S_kernel = 0.5 * phi_L[i] * compute_stress_kernel(grad_phi_j_R, -normal);
                    const auto& SIP_RR_S_kernel = 0.5 * phi_R[i] * compute_stress_kernel(grad_phi_j_R, normal);
                    /* 扩散 SIP  惩罚项 */
                    const auto& SIP_P_kernel = penalty * compute_stress_kernel(normal, normal);
                    const auto& SIP_LL_P_kernel = 0.5 * phi_L[i] * phi_L[j] * SIP_P_kernel;
                    const auto& SIP_LR_P_kernel = 0.5 * phi_R[i] * phi_L[j] * -SIP_P_kernel;
                    const auto& SIP_RL_P_kernel = 0.5 * phi_L[i] * phi_R[j] * -SIP_P_kernel;
                    const auto& SIP_RR_P_kernel = 0.5 * phi_R[i] * phi_R[j] * SIP_P_kernel;

                    assemble_Auu(mat_LL, (SIP_LL_kernel + SIP_LL_S_kernel - SIP_LL_P_kernel) * jac_weight, j, i);
                    assemble_Auu(mat_LR, (SIP_LR_kernel + SIP_LR_S_kernel - SIP_LR_P_kernel) * jac_weight, j, i);
                    assemble_Auu(mat_RL, (SIP_RL_kernel + SIP_RL_S_kernel - SIP_RL_P_kernel) * jac_weight, j, i);
                    assemble_Auu(mat_RR, (SIP_RR_kernel + SIP_RR_S_kernel - SIP_RR_P_kernel) * jac_weight, j, i);
                }
...
assemble(const ComputingMesh& mesh, const LongVector<DoFs>& old_solution,
            ...
            LongVector<DoFs>& sparse_rhs){
    // print("123");
    assemble_cells(mesh,old_solution,sparse_mat);  // 这一部分是对称矩阵
    // print("234");
...
template<uInt OrderU, uInt OrderP, typename Flux, typename GaussQuadCell, typename GaussQuadFace>
vector3f InCompressibleImplicitDiffusion<OrderU, OrderP, Flux, GaussQuadCell, GaussQuadFace>::
...
// 计算应力张量乘积核 (体积分/面积分通用)
template<uInt OrderU, uInt OrderP, typename Flux, typename GaussQuadCell, typename GaussQuadFace>
DenseMatrix<3,3> InCompressibleImplicitDiffusion<OrderU, OrderP, Flux, GaussQuadCell, GaussQuadFace>::
compute_stress_kernel(
    ...
    // 对角线项
    result(0,0) = 2 * left_vec[0] * right_vec[0];
    result(1,1) = 2 * left_vec[1] * right_vec[1];
    result(2,2) = 2 * left_vec[2] * right_vec[2];
    
    // 非对角线项 (对称填充)
    result(0,1) = left_vec[1] * right_vec[0] + left_vec[0] * right_vec[1];
    result(1,0) = result(0,1);
    
    result(0,2) = left_vec[2] * right_vec[0] + left_vec[0] * right_vec[2];
    result(2,0) = result(0,2);
    
    result(1,2) = left_vec[2] * right_vec[1] + left_vec[1] * right_vec[2];
