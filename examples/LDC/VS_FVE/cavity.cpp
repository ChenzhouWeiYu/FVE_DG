...
void Run(uInt N, FilesystemManager fsm){
    ...
    **   设置初值
    ...
    // LongVector<DoFs> U_n = read_solution_file<QuadC,Basis>(cmesh, "./T_18_N_12.txt");
    ...
    **   间隔 Dt 时间保存一次
    ...
    for(uInt save_step = 18; save_step < 400; save_step++){    
        Scalar Dt = 1.0;
...
//                LongVector<DoFs> U_k_tmp = solver.DGMRES(U_k);
		...
		//if (picard_iter>5 || total_time < 0.1){
		...
		auto [krylov_iter, krylov_residual] = solver.DGMRES(U_k, U_k_tmp);
                {
                    std::ostringstream oss;
                    oss << krylov_residual;
                    logging("Krylov Iter: " + std::to_string(krylov_iter) + "  \t residual: " + oss.str());
                }
